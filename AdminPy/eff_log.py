...
cols=['kua','adm','air','eia','WWe','WWc','WWp','WWs','SNW','WST','CON','ONM','IND']
dpts=['adm','adm','E1','E1','WW','WW','WW','WW','E2','E2','ER','ER','ER']
c2d={i:j for i,j in zip(cols,dpts)}
for d in set(dpts):
  p=pv2c
  for c in cols:
    if c not in pv2.columns:continue
    if c2d[c]==d:
      p=[i+j for i,j in zip(p,list(pv2.loc[:, c]))]
  pv2[d]=p
for d in set(dpts):
  if d not in pv2.columns:continue
  pv2c=[i+j for i,j in zip(pv2c,list(pv2.loc[:, d]))]
  ax.plot(totalSeed, pv2c, label=d,alpha=0.7)
...
if act[0] == 'save'[0]:
    name = '/home/py_exam/openKM_log/orquestra/static/eff_logs.png'
    plt.savefig(name)
